...
class TweakHandler {
	...
	public func getInputFiles() async throws {
		...
		// The extracted members are only ever stepping stones into the app
		// bundle: whichever way this ends — success, a bad deb, an injection
		// error — the directory they were unpacked into goes with them. It used
		// to survive until the next launch's tmp sweep.
	...
	private func _handleDeb(at url: URL, baseTmpDir: URL) async throws {
		...
		for arFile in arFiles {
			// an ar member gets to name its own path, and the deb it came out of
			// is untrusted, so the name has to stay inside the subdirectory
			guard !isUnsafeArchiveMember(arFile.name, base: uniqueSubDir) else {
				throw TweakHandlerError.unsafeArchiveMember(arFile.name)
			}
...
// tars and ars both let every member name its own path, and nothing stops that
// path from climbing out of the directory the archive is being unpacked into
func isUnsafeArchiveMember(_ name: String, base: URL) -> Bool {
	// both separators are legal, and one of them can be used to hide the other
	...
	return resolved != basePath && !resolved.hasPrefix(basePath + "/")
}
...
enum TweakHandlerError: Error, LocalizedError {
	...
	/// What the user is told. These reach a notification, a popup and the
	/// activity log, so they are sentences rather than the enum's own
	/// description — "The operation couldn't be completed. (BatSign.
	/// TweakHandlerError error 3.)" is not a reason anyone can act on.
	var errorDescription: String? {
		switch self {
		case .unsupportedFileExtension(let name):
			"\(name) is not a format this app can inject."
		case .decompressionFailed(let name):
			"\(name) could not be unpacked — the file may be damaged."
		case .missingFile(let name):
			"\(name) does not contain a file the injection needs."
		case .noAccess:
			"The tweak file could not be read."
		case .unsafeArchiveMember(let name):
			"The tweak tries to write outside its own folder (\(name)), so it was refused."
		}
	}
}
