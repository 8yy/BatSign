...
struct BackupRestoreView: View {
	...
	/// Every preference key BatSign owns, in both directions.
	private static let _managedKeys: [String] = [
		"BatSign.autoUpdateEnabled",
		"BatSign.autoUpdateInterval",
		"BatSign.autoUpdateWifiOnly",
		"BatSign.autoUpdateNightOnly",
		"BatSign.autoSignEnabled",
		"BatSign.autoRenewEnabled",
		"BatSign.autoDeleteOldVersions",
		"BatSign.notificationsEnabled",
		"BatSign.badgeUpdates",
		"BatSign.biometricLock",
		"BatSign.perAppAutoUpdate",
		"BatSign.sourceAutoUpdate",
		"BatSign.recentSearches",
		...
		NBNavigationView(.localized("Backup & Restore")) {
			Form {
				NBSection(.localized("Backup")) {
					...
					Text(.localized("Exports your repositories and all BatSign preferences to a file. Certificates are not included."))
				}
...
extension BackupRestoreView {
	...
	private func _export() {
		...
		do {
			...
			let url = URL.documentsDirectory.appendingPathComponent("BatSign-Backup-\(stamp).json")
	...
	private func _import(_ url: URL) {
		...
		do {
			...
			guard
				...
				_statusMessage = .localized("Not a valid BatSign backup file.")
				return
			}
