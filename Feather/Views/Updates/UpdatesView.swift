...
struct UpdatesView: View {
	...
	private func _updateCard(_ update: AppUpdate) -> some View {
		HStack(spacing: WSSpacing.md) {
			...
			// The row has to answer for the work it started. Nothing here used
			// to observe the transfer, so a tapped row stayed exactly as it was
			// — "Update" and all — while the same job was already running under
			// it, and the only place the download showed was the activity list.
			if let download = downloadManager.getDownload(by: "\(BatSignAuto.manualUpdatePrefix)_\(update.localUUID)") {
				WSActionButton(title: "Downloading", isDisabled: true) {}
					.opacity(0.6)
			} else {
					...
					ForEach(rows, id: \.identifier) { row in
						HStack(spacing: WSSpacing.md) {
							...
							WSActionButton(title: row.held ? "Resume" : "Unskip", style: .quiet) {
								...
								// The optimistic restore re-adds what the last check
								// found. A fetch replaces it with the truth: an app the
								// user has since updated by hand must not return as a
								// stale update with a live Update button.
								Task { await autoUpdateManager.refreshUpdatesSilently() }
							}
						}
						.padding(WSSpacing.cardPadding)
						.background(WSRadius.continuous(WSRadius.lg).fill(WSSurface.card))
