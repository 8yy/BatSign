		...
		ZStack {
			if !downloadManager.manualDownloads.isEmpty {
				VStack {
					...
					.padding(.vertical, 6)
				}				// Real Liquid Glass — the same single-surface treatment as the
				// system's own floating bars. The old `.regularMaterial` plus a
				// hand-drawn stroke read as a flat grey slab with a fake outline:
				// two layers pretending to be one.
				.bsGlass(cornerRadius: 22)
