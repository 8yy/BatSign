...
struct WSActionButton: View {
	...
	private var _foreground: Color {
		switch style {
		// The bar's own buttons are chrome, and chrome on this app's dark
		// ground is type, not colour: a blue label was the one thing on the
		// Library and Updates screens that read as a foreign element against
		// every other control in the app's own palette. The tint still carries
		// a *prominent* button, which is a call to action rather than a
		// control sitting in a row.
...
struct WSProgressBadge: View {
	...
	var body: some View {
		ZStack {
			Circle()
				.stroke(BS.chipFill, lineWidth: 3)
			Circle()
