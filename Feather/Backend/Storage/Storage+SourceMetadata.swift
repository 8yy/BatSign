...
extension Storage {
	...
	func addSourceMetadata(
		...
		// Reached from the import and signing pipelines, which run detached. The
		// fetch and the insert both belong to the context's own queue — see
		// `Storage.perform`.
		perform {
			let metadata = self.sourceMetadata(for: appUUID)
				?? AppSourceMetadata(context: self.context)
			let now = Date()
	...
	func copySourceMetadata(
		...
		guard let sourceAppUUID else {
			return
		}

		// Same as `addSourceMetadata`: this is called from the signing pipeline's
		// detached task, so the read and the write both move to the context's
		// queue together.
		perform {
			guard let source = self.sourceMetadata(for: sourceAppUUID) else {
				return
			}
			...
			let metadata = self.sourceMetadata(for: destinationAppUUID)
				?? AppSourceMetadata(context: self.context)
			let now = Date()
			if metadata.createdAt == nil {
				metadata.createdAt = now
