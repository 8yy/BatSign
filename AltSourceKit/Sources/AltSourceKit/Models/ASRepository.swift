...
extension ASRepository {
	public struct App: Sendable, Decodable, Hashable, Identifiable {
		...
		public struct Screenshots: Decodable, Hashable, Sendable {
			/// One app shot: its URL, and the pixel size the source declared for it
			/// when it declared one. The size matters — the store lays its shot
			/// carousel out at a fixed height, so a shot with no declared shape can
			/// only be drawn once it has loaded.
			public struct Shot: Decodable, Hashable, Sendable {
				public var url: URL
				...
				/// Width over height, when the source published both.
				...
				public init(from decoder: any Decoder) throws {
					// A bare URL string is the simplest shape a source publishes.
					...
					let container = try decoder.container(keyedBy: CodingKeys.self)
					var raw = try container.decodeIfPresent(String.self, forKey: .url)
					if raw == nil {
						raw = try container.decodeIfPresent(String.self, forKey: .imageURL)
					}
					guard let raw, let url = URL(string: raw) else {
						throw DecodingError.dataCorruptedError(
							forKey: .url,
							in: container,
							debugDescription: "a screenshot without a usable URL"
						)
					}
					self.url = url
					self.width = Self.number(in: container, forKey: .width)
					self.height = Self.number(in: container, forKey: .height)
				}

				/// A dimension a source may publish as either an integer or a float.
				...
				public enum CodingKeys: String, CodingKey {
					case url, imageURL, width, height
				}
			}

			public var iPhone: [Shot]?
			public var iPad: [Shot]?

			public init(iPhone: [Shot]? = nil, iPad: [Shot]? = nil) {
				self.iPhone = iPhone
				self.iPad = iPad
			}

			/// One entry that survives being unreadable.
			///
			/// A shot that carries no usable URL is dropped rather than thrown:
			/// throwing here would fail the whole app record, so one malformed
			/// screenshot in a repository would take the entire app with it.
			private struct LenientShot: Decodable {
				let shot: Shot?

				init(from decoder: any Decoder) throws {
					shot = try? Shot(from: decoder)
				}
			}
			...
			public init(from decoder: any Decoder) throws {
				...
				// One, two and three all arrive as a bare array of shots.
				if let list = try? decoder.singleValueContainer().decode([LenientShot].self) {
					let shots = list.compactMap(\.shot)
					self.iPhone = shots.isEmpty ? nil : shots
					self.iPad = nil
					return
				}

				// Four: split by the device the shots were taken on.
				let container = try decoder.container(keyedBy: CodingKeys.self)
				let phone = (try? container.decodeIfPresent([LenientShot].self, forKey: .iPhone)) ?? nil
				let pad = (try? container.decodeIfPresent([LenientShot].self, forKey: .iPad)) ?? nil
				let phoneShots = phone?.compactMap(\.shot) ?? []
				let padShots = pad?.compactMap(\.shot) ?? []
				self.iPhone = phoneShots.isEmpty ? nil : phoneShots
				self.iPad = padShots.isEmpty ? nil : padShots
			}
		...
		/// The app's shots, whichever shape the source published them in: the
		/// device-split object first, then the flat URL array, so a page never
		/// ends up with an empty Preview because a source chose the other form.
		public var allScreenshots: [Screenshots.Shot] {
			if let phone = screenshots?.iPhone, !phone.isEmpty { return phone }
			if let pad = screenshots?.iPad, !pad.isEmpty { return pad }
			return (screenshotURLs ?? []).map { Screenshots.Shot(url: $0) }
		}
